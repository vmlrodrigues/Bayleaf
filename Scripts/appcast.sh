...
APP="$ROOT/dist/Bayleaf.app"
DMG="$ROOT/dist/Bayleaf.dmg"
APPCAST="$ROOT/appcast.xml"
REPO="${GITHUB_REPO:-vmlrodrigues/Bayleaf}"
ACCOUNT="${SPARKLE_ACCOUNT:-Bayleaf}"
...
# then contradicts the published binary. Not hypothetical: four ClawBar releases shipped
# with the tag on the wrong commit and had to be moved by hand — hence these guards.
#
...
# reporting success.
#
# Not a theoretical gap either: a ClawBar release went out with its tag on the wrong
# commit for exactly this reason, minutes after the other three guards were added.
# Local correctness felt like enough right up until it wasn't.
...
# releases/latest/download/Bayleaf.dmg, which resolves only if an asset is named exactly
...
To publish:
  ...
  gh release create "v$VERSION" \\
      "$STAGED#Bayleaf $VERSION (macOS, notarised)" \\
      "$STABLE#Bayleaf (latest, Apple Silicon)" \\
      --repo "$REPO" --title "Bayleaf $VERSION" --notes "…"
